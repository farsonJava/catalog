...
# Note:
Our equals overriding method is still makes sure that the object its comparing is the same class, (as evidenced in the screenshot above). The cartManager allows us to keep this "type-safe" equals overriding system and maintain a loose relationship with the User object; It uses a string that asks for an email property and derives it from our secured session attribute whilst Usermanager's user object's equals override method ensures we only achieve equality comparing the <i> same class, or object </i>,  since we know that only one account can have a given email, so conceptually if two user objects have the same email we can equate them- so this shows that this is also a way in which we reinforce an object's or entitie's characteristics. In contrast, the reason the cartItems within the CartManager hashmap themselves are overridenn in the same type-safe way for the hashmap to function is for the same reason as we previously stated; to increment a value for the same key even when its derived from the <i> different objects of the same class </i> with differing memory addresses but conceptually valid comparison properties such as its name. Using strings as authentication properties however, is not production grade standard as you must now be hyper-vigilant of where you allow user inputs as you can be subject to string injection vulnerabilities.)
