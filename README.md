...
from them although it is logically related since the retrieved attribute after login is the "email" property, and this is used to access our cartManager hashmap. There is a logical consistency in referencing the user instance in terms of validating it using the same session attribute as we use to access keys from the CartManager hashmap. 

In this design sensitive transactions are accessed by having your email stored in session state after you have logged in with a conceptually <i>private</i> property of User called password. Through a hashmap key common to Users, the CartManager class can be logically related to them while being decoupled as an object in code, and therefore it may facilitate sensitive transactions associated with respective user accounts while remaining a seperate concern, and yet its still part of the authentication flow by virtue of the fact that the design ensures the user email only reaches the CartServlet endpoints after the user inputs their private data in the form of a password at login through a property of the session object. This way, it isn't using sensitive authenticated data like a password as a key for ensuring a specific user is targeted for a transaction. (In a real production application, passwords are also hashed after the same concern). This allows us to see how .hashcode() and .equals() overriding can make our application more secure through decoupling.  

#Note:
Our equals overriding method is predicated on the principle that the object its comparing is the same class, (as evidenced in the screenshot above). The cartManager allows us to keep this "type-safe" equals overriding system and maintain a loose relationship with the User object; It uses a string that asks for an email property and derives it from our saved session attribute whilst Usermanager's user object's equals override method ensures we only achieve equality comparing the <i> same class, or object </i>,  since we know that only one account can have a given email, so conceptually if two user objects have the same email we can equate them. This is also a way in which we reinforce an object's or entities characteristics, whilst in the case of the cartManager class we maintain a loose coupled relationship with it by being satisfied with a string that is derived from login logic in session attributes for our key: However the cartItems themselves are overridenn in the same type-safe way for the hashmap to function as we previously stated; to increment a value for the same key even when its derived from the <i> different objects of the same class </i> with differing memory addresses but conceptually valid comparison properties such as its name. Using strings as authentication properties however, is not production grade standard as you must now be hyper-vigilant of where you allow user inputs as you can be subject to string injection vulnerabilities.)
...
implement lower level designs has given me greater control and confidence with higher level frameworks that streamline or re-invent some these processes. 

Having worked on a non-REST API endpoint design highlights some of the advantages of a resource-based request system, especially regarding convoluted coupling / security issues generated by server-side session data and poorly scaling action based request 
