...
Having worked on a non-REST API endpoint design highlights some of the advantages of a resource-based request RPC system, especially regarding convoluted coupling / security issues generated by server-side session data and poorly scaling action based request 
...
1.) If server memory stores individual client data, it may only serve one client (as would be the case within this design)
...
controls which individual data it wants to expose on a request basis encapsulation becomes simpler.

Additionally, the maintability of a codebase adhering to rest constraints is far greater so far as resource based requests can adhere to HTTP semantics. Our "POST" request routing system isn't an idiomatic usage of HTTP rather than a hack or a workaround
for containing all access servlet logic. 
